...
def get_chat_db_connection():
    ...
    if LIVE_DB_CONNECTION is None:
        ...
        LIVE_DB_CONNECTION = sqlite3.connect(get_live_chat_db_path(), timeout=1.0, check_same_thread=False)
...
def to_nanos(raw: int) -> int:
    ...
    if not isinstance(raw, int): # Guard against non-integer inputs
        logger.warning(f"to_nanos received non-integer input: {raw} of type {type(raw)}")
...
def fetch_new_messages_by_date(last_apple_epoch: int) -> List[Dict]:
    ...
    try:
        ...
        for m in messages:
            if 'date' in m and m['date'] is not None:
                try:
                    ...
                    logger.error(f"Could not convert date {m['date']} to int for to_nanos: {e}")
        ...
        #     if valid_dates:
        #         logger.info(f"[LiveSync] fetch_new_messages → {len(messages)} rows "
        ...
        #     else:
        #         logger.info(f"[LiveSync] fetch_new_messages → {len(messages)} rows, but no valid integer dates found after normalization.")
        # else:
        #     logger.debug(f"[LiveSync] No new messages since watermark_ns={last_apple_epoch} (queried as chat.db units ~{query_timestamp})")
        return messages
    except sqlite3.Error as e:
        logger.error(f"Error fetching new messages: {e}", exc_info=True)
...
def fetch_new_messages(last_rowid: int, last_apple_epoch: int = None) -> List[Dict]:
    ...
    try:
        ...
        if messages:
            logger.info(f"[LiveSync] fetch_new_messages → {len(messages)} rows using ROWID > {last_rowid}, max_rowid={max_rowid}")
        
        return messages, max_rowid
    except sqlite3.Error as e:
        logger.error(f"Error fetching new messages by ROWID: {e}", exc_info=True)
        # If there's an error with the connection, reset it so it will be recreated on next call
        global LIVE_DB_CONNECTION
        try:
            if LIVE_DB_CONNECTION:
...
def fetch_new_attachments_by_date(last_apple_epoch: int) -> List[Dict]:
    ...
    try:
        ...
        # logger.debug(f"Extracted {len(attachments)} new attachments from {LIVE_DB}") # Commented out
        return attachments
    except sqlite3.Error as e:
        logger.error(f"Error fetching new attachments: {e}", exc_info=True)
        # If there's an error with the connection, reset it so it will be recreated on next call
        global LIVE_DB_CONNECTION
        try:
            if LIVE_DB_CONNECTION:
...
def fetch_new_attachments(last_rowid: int) -> List[Dict]:
    ...
    try:
        ...
        if attachments:
            logger.info(f"[LiveSync] fetch_new_attachments → {len(attachments)} rows using ROWID > {last_rowid}, max_rowid={max_rowid}")
        
        return attachments, max_rowid
    except sqlite3.Error as e:
        logger.error(f"Error fetching new attachments by ROWID: {e}", exc_info=True)
        # If there's an error with the connection, reset it so it will be recreated on next call
        global LIVE_DB_CONNECTION
        try:
            if LIVE_DB_CONNECTION:
...
def fetch_new_chats(last_apple_epoch: int = 0) -> List[Dict]:
    ...
    try:
        ...
        # logger.debug(f"Extracted {len(chats)} chats with new activity from {LIVE_DB}") # Commented out
        return chats
    except sqlite3.Error as e:
        logger.error(f"Error fetching new chats: {e}", exc_info=True)
        # If there's an error with the connection, reset it so it will be recreated on next call
        global LIVE_DB_CONNECTION
        try:
            if LIVE_DB_CONNECTION:
