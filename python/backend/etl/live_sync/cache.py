...
from backend.etl.utils import normalize_phone_number # Assuming this is in your project structure
...
def update_contact_cache(identifier: str, contact_id: int):
    """
    Update a single entry in the contact cache.
    Thread-safe operation.
    
    Args:
        identifier: The contact identifier (normalized)
        contact_id: The contact ID to store
    """
    with _cache_lock:
        CONTACT_MAP_CACHE[identifier] = contact_id
        logger.debug(f"Updated contact cache: {identifier} -> {contact_id}")

def remove_contact_from_cache(identifier: str):
    """
    Remove a single entry from the contact cache.
    Thread-safe operation.
    
    Args:
        identifier: The contact identifier to remove
    """
    with _cache_lock:
        if identifier in CONTACT_MAP_CACHE:
            del CONTACT_MAP_CACHE[identifier]
            logger.debug(f"Removed contact from cache: {identifier}")

def get_contact_from_cache(identifier: str) -> Optional[int]:
    """
    Get a contact ID from cache in a thread-safe manner.
    
    Args:
        identifier: The contact identifier to look up
        
    Returns:
        contact_id if found, None otherwise
    """
    with _cache_lock:
        return CONTACT_MAP_CACHE.get(identifier)
...
def get_contact_map(cursor, force_refresh=False):
    ...
    if not CONTACT_MAP_CACHE or not _contacts_cache_populated or force_refresh: # Check CONTACT_MAP_CACHE for emptiness too
        ...
        logger.debug("Refreshing contact_map_cache from DB")
        cursor.execute("""
...
def get_message_guid_to_id_map(cursor, force_refresh=False):
    ...
    if not MESSAGE_GUID_TO_ID_CACHE or not _message_guid_cache_populated or force_refresh: # Check MESSAGE_GUID_TO_ID_CACHE for emptiness
        ...
        logger.debug("Refreshing message_guid_to_id_cache from DB")
...
def get_chat_map(cursor, force_refresh=False):
    ...
    if not CHAT_MAP_CACHE or not _chat_map_cache_populated or force_refresh:
        ...
        logger.debug("Refreshing chat_map_cache from DB")
...
def get_reaction_guids(cursor, force_refresh=False):
    ...
    if not REACTION_GUID_CACHE or not _reaction_guid_cache_populated or force_refresh:
        ...
        logger.debug("Refreshing reaction_guid_cache from DB")
...
def get_attachment_guids(cursor, force_refresh=False):
    ...
    if not ATTACHMENT_GUID_CACHE or not _attachment_guid_cache_populated or force_refresh:
        ...
        logger.debug("Refreshing attachment_guid_cache from DB")
...
def reset_contact_cache():
    """Reset only the contact cache, forcing it to repopulate on next get."""
    global _contacts_cache_populated
    with _cache_lock:
        logger.debug("Resetting contact cache")
        CONTACT_MAP_CACHE.clear()
        _contacts_cache_populated = False

def reset_message_guid_map_cache():
    """
    Reset only the message GUID to ID cache, forcing it to repopulate on next get.
    """
    global _message_guid_cache_populated
    with _cache_lock:
        MESSAGE_GUID_TO_ID_CACHE.clear()
...
def reset_all_caches():
    ...
    with _cache_lock:
        logger.debug("Resetting all live_sync caches")
        CONTACT_MAP_CACHE.clear()
        _contacts_cache_populated = False
        MESSAGE_GUID_TO_ID_CACHE.clear()
        _message_guid_cache_populated = False
