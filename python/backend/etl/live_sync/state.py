...
def set_watermark(value: int):
    ...
    with db.session_scope() as session:
        ...
        logger.debug(f"Watermark updated to {value}")
...
def initialize_watermark_if_missing():
    ...
    if watermark is None:
        logger.info("No watermark found, initializing from existing messages (in nanoseconds)")
        with db.session_scope() as session:
            cursor = session.connection().connection.cursor()
                ...
                if isinstance(max_ts_row[0], str):
                    try:
                        ...
                        if max_ts_datetime is None:
                            logger.error(f"Could not parse timestamp string: {max_ts_row[0]}")
            ...
            if max_ts_datetime:
                ...
                logger.info(f"Initializing watermark to {watermark} ns from max message timestamp ({max_ts_datetime})")
            else:
                ...
                watermark = int(watermark_seconds * 1_000_000_000) # Convert to nanoseconds
                logger.info(f"No messages found, initializing watermark to {watermark} ns (1 day ago)")
...
def initialize_rowid_watermarks_if_missing():
    ...
    if message_rowid == 0 and attachment_rowid == 0:
        ...
        if max_message_rowid:
            ...
            logger.info(f"Initialized message ROWID watermark to {max_message_rowid}")
        else:
            set_message_rowid_watermark(0)
            logger.info("No messages found in chat.db, initialized message ROWID watermark to 0")
        ...
        if max_attachment_rowid:
            ...
            logger.info(f"Initialized attachment ROWID watermark to {max_attachment_rowid}")
        else:
            set_attachment_rowid_watermark(0)
            logger.info("No attachments found in chat.db, initialized attachment ROWID watermark to 0")
