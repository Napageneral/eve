...
from backend.etl.etl_attachments import _convert_apple_timestamp
...
def sync_attachments(new_rows: List[Dict]) -> int:
    ...
    with db.session_scope() as session:
        ...
        logger.debug("Building cache maps for attachments sync")
        ...
        for row in new_rows:
            ...
            if not message_id:
                logger.warning(f"Message not found for attachment: {row['guid']}")
                skipped_count += 1
                continue
        ...
        if attachments_to_insert:
            logger.info(f"Inserting {len(attachments_to_insert)} new attachments")
            cursor.executemany("""
        ...
        if imported_count > 0 or skipped_count > 0:
            logger.info(f"Attachments sync complete - Imported: {imported_count}, Skipped: {skipped_count}")
